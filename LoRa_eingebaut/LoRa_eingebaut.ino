...
// ======== Busy-Detection (CCA) ========
#define CCA_RSSI_THRESH_DBM -90            // über dieser Schwelle gilt der Kanal als belegt -- KALIBRIEREN!
#define CCA_BACKOFF_MS      100            // einmaliges Warten, wenn der Kanal belegt ist

// ======== Pins am Feather M0 (Standard-Feather-M0-RFM95-Belegung) ========
...
// Kompaktes Binär-Payload: 3 Bytes statt CSV-Text -> kürzere Airtime
struct __attribute__((packed)) TempPayload {
  uint8_t id;                              // Sender-ID (1 Byte)
  int16_t th;                              // Thermoelement-Temp × 10 (0,1 °C Auflösung, 2 Bytes)
};
...
static void readFresh(double &th, double &tc) {  // th=Thermocouple, tc=Ambient
  ...
  delay(90);                               // Wartezeit für frische Konversion (ausreichend bei 14-Bit-ADC)
...
// ---------- Kurz zuhören: sendet gerade jemand? ----------
static bool channelBusy() {
  rf95.setModeRx();                   // Empfänger einschalten
  delay(2);                           // kurz einschwingen lassen (PLL/AGC)
  int16_t peak = -200;                // höchsten Pegel im Lauschfenster suchen
  for (uint8_t i = 0; i < 10; i++) {  // ~2,5 ms lauschen
    int16_t r = chanRssiDbm();
    if (r > peak) peak = r;
    delayMicroseconds(250);
  }
  rf95.setModeIdle();                 // Empfänger wieder in Standby
...
static void doMeasurementAndSend() {
  ...
  // Payload formatieren (binär, kompakt)
  TempPayload payload;
  payload.id = (uint8_t)ID;
  payload.th = (int16_t)round(th * 10.0f); // z.B. 23,9 °C -> 239

  // --------- Busy-Detection: kurz hineinhören ----------
...
void setup() {
  ...
  mcp.setThermocoupleType(MCP9600_TYPE_J); // Thermoelement-Typ konfigurieren (hier: Typ J)
  mcp.setFilterCoefficient(3);             // Digitalfilter (0–7): Glättung vs. Reaktionszeit
  mcp.setADCresolution(MCP9600_ADCRESOLUTION_14);  // 14-bit ADC-Auflösung (0,25 °C, schnelle Wandlung)
  mcp.setAmbientResolution(RES_ZERO_POINT_25);     // Ambient-Auflösung auf 0,25 °C
