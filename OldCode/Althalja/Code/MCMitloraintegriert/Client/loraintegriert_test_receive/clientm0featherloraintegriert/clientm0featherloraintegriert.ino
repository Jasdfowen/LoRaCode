...
static void doMeasurementAndSend() {
  ...
  snprintf(msg, sizeof(msg), "%.1f,%.1f,%.1f", th, tc, dT); // CSV-Format mit 1 Nachkommastelle
...
//Empfangsbestätigung
// ======== RX-Konfiguration ========
#define RX_TIMEOUT_MS 100    // Timeout in ms – mind. ~60 ms für SF7/125kHz (ein Paket ≈ 57 ms)

// ---------- RX mit Timeout ----------
// Gibt true zurück wenn eine Nachricht empfangen wurde, false bei Timeout
static bool waitForMessage(char* buf, uint8_t bufLen) {
  rf95.setModeRx();                          // Empfänger einschalten
  uint32_t t0 = millis();
  while (millis() - t0 < RX_TIMEOUT_MS) {
    if (rf95.available()) {                  // Paket angekommen?
      uint8_t len = bufLen - 1;              // max. Nutzlänge (Platz für '\0' lassen)
  ...
  rf95.sleep();                              // Timeout – Empfänger abschalten
  return false;
}

// ---------- Nachricht verarbeiten ----------
// Erwartet Schema: "N <Zahl>", z.B. "N 1", "N 42"
static void processMessage(const char* msg) {
  int n = 0;
  if (sscanf(msg, "N %d", &n) == 1) {
    // Gültige Nachricht empfangen – Verarbeitung hier einfügen
    // n enthält die empfangene Zahl
    // (vorerst leer – nur Empfang + Parse testen)
  }
  // Kein sscanf-Match → Schema passt nicht → ignorieren
}
...
void setup() {
  ...
  // ---------- RX-Fenster: auf "N <Zahl>" warten ----------
  char rxBuf[40];
  if (waitForMessage(rxBuf, sizeof(rxBuf))) {
  }
  // Bei Timeout läuft der Code hier einfach weiter → DONE an TPL5111
